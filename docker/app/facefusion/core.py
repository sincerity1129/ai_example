...
from facefusion.streamlit_controller import remove_file
...
def cli(sourch_path, target_path, gpu='cpu', face_enhancer_mode="face_swapper"):
	program = ArgumentParser(formatter_class = lambda prog: HelpFormatter(prog, max_help_position = 120), add_help = False)
	# general
	program.add_argument('-s', '--source', default=sourch_path, help = wording.get('source_help'), dest = 'source_path')
	program.add_argument('-t', '--target', default=target_path, help = wording.get('target_help'), dest = 'target_path')
	program.add_argument('-o', '--output', default="/workspace/data/output",help = wording.get('output_help'), dest = 'output_path')
	...
	group_execution.add_argument('--execution-providers', help = wording.get('execution_providers_help').format(choices = 'cuda'), dest = 'execution_providers', default = [ gpu ], choices = encode_execution_providers(onnxruntime.get_available_providers()), nargs = '+')
	...
	if face_enhancer_mode == "face_enhancer":
	...
	run(program)
	remove_file(demo['input_root_path'])
...
def process_image() -> None:
	...
	# process frame
	reset_list =[]
	for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors, reset_list):
